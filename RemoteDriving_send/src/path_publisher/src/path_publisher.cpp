...
double lookahead_distance_ = 3;
int gps_times = 0;
float give_speed = 0.0;
...
public:
        ...
        : last_lookahead_index_(0), robot_yaw(0.0) // 初始化 last_lookahead_index_ 为 0，robot_yaw 为 0
    {
        ...
        liangche_cmd_vel_pub = nh.advertise<geometry_msgs::Twist>("/liangche_cmd_vel", 10);
    ...
    void readgpsCallback(const nav_msgs::Path::ConstPtr &msg) {
        ...
        for (size_t i = 0; i < msg->poses.size(); ++i) {
            ...
            double y = pose.pose.position.y - 10.0;
            path_points_.emplace_back(x, y);

            if (gps_times > 0 && gps_times%2 == 0) {
                double dx = x - last_point.first;
                double dy = y - last_point.second;
                ...
                ROS_INFO("Distance between last point and current point: %f ", distance);
                ROS_INFO("speed: %f ", give_speed);
    ...
    void compute_angle(const std::pair<double, double> &lookahead_point) {   
        if(count == 0) 
        {
            std::cout << " wait for gps data " << std::endl;
            return;
        }
        bool back_val = compute_reverse_forward();
        cmd_msg.linear.x = back_val ? -1 : 1;
        double dist11 = path_points_[path_points_.size() - 1].first - vehicle_pos_.x;
        double dist22 = path_points_[path_points_.size() - 1].second - vehicle_pos_.y;
        liangche_cmd_msg.linear.x = sqrt(dist11 *dist11 + dist22 * dist22);
        if(liangche_cmd_msg.linear.x > 7 && liangche_cmd_msg.linear.x < 10){
                lookahead_distance_ = 4;
           

        } else if(liangche_cmd_msg.linear.x > 5 && liangche_cmd_msg.linear.x < 7){
                lookahead_distance_ = 3.2;
        }else{
            lookahead_distance_ = 3;
        }
        ...
        // if (fabs(delta1) > 15 && fabs(delta1) <= 25) delta1 = delta1 / 45 ;
        // else if (fabs(delta1) > 25 && fabs(delta1) <= 30) delta1 = delta1 / 45;
        // else if (fabs(delta1) > 30 && fabs(delta1) <= 45) delta1 = delta1 / 45;
        // else delta1 = delta1 / 45;
        // if(fabs(delta1) > max_orien_ange) delta1 = (fabs(delta1) / delta1) * 1;
        // delta1 = delta1 * 500;

        cmd_msg.angular.z = delta1;
        cmd_msg.linear.x = give_speed;
        // ROS_INFO("angle: %f , speed: %f ", cmd_msg.angular.z, cmd_msg.linear.x);

    // ros::Publisher liangche_cmd_vel_pub;
    // geometry_msgs::Twist liangche_cmd_msg;

        // std::cout << " dist : " << liangche_cmd_msg.linear.x
        // << " lookahead_distance_ " << lookahead_distance_ <<std::endl;
        liangche_cmd_vel_pub.publish(liangche_cmd_msg);
    ...
    void poseCallback(const geometry_msgs::Pose2D::ConstPtr &msg) {
        ...
        // std::cout << "poseCallback: Vehicle position updated." << std::endl;
        visualize();
    }
    ...
    void updateLookaheadPoint() {
        std::cout << "path_points_.size(): " << path_points_.size() << std::endl;
        for (size_t i = last_lookahead_index_; i < path_points_.size(); ++i) {
             ...
             * (path_points_[path_points_.size() - 1].first - vehicle_pos_.x)
              ...
              * (path_points_[path_points_.size() - 1].second - vehicle_pos_.y));
            ROS_WARN_THROTTLE(1, "distance: %f", distance1);
            // std::cout << "hapi"  << distance << std::endl;
            // 判断是否接近最后一个预瞄点
