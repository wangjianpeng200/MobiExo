...
//       delta1 = atan2(2 *L * sin(delta1 * M_PI / 180.0), lookahead_distance_);
//       delta1 = delta1 * 180 / M_PI;
//       delta1 = clamp(delta1, -45.0, 45.0);
//       // ROS_ERROR_THROTTLE(1, "Angle theta: %f ",delta1 );
//       if(fabs(delta1) > 15 &&fabs(delta1) <= 25)  delta1 = delta1 * 12.2;
//       else if(fabs(delta1) > 25 && fabs(delta1) <=30)  delta1 = delta1 * 15.33;
//       // else if(fabs(delta1) > 0 && fabs(delta1) <=15)  delta1 = delta1 * 38.5;
//       else  delta1 = delta1 * 8.88;
//       // delta1 = (delta1 / 45) * 580;
//       ROS_ERROR_THROTTLE(1, "Angle theta: %f ",delta1 );

//       if(back_val == true){
//         cmd_msg.angular.z = -delta1;
//         ROS_WARN_THROTTLE(1, "back");
//       }else{
//         cmd_msg.angular.z = delta1;
//         ROS_WARN_THROTTLE(1, "forword");
...
//       // ROS_INFO("Extended Point: x = %f, y = %f", extension_x, extension_y);
...
//         compute_angle(lookahead_point_); // 立即计算角度，避免越界
//         break;
//       }
//     }
//   }
...
//       // 设置头信息
//       pose_stamped.header.frame_id = "map";  // 坐标系设为 "map"
//       pose_stamped.header.stamp = ros::Time::now();  // 当前时间戳
