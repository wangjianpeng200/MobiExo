...
ros::Publisher ass_maker_pub, pub_lane_marker, obj_marker_pub, ukf_maker_pub, plan_lane_marker_pub, v2x_car_model_pub;
...
void ass_callback(const custom_msgs::ObjectArray &ass_data)
{
    ...
    for(int i = 0; i < ass_data.objs.size(); i++)
    {
       ...
       // AssMarker.draw_text(ass_center, std::to_string(ass_data.objs[i].id), i, assmarker, white);
        AssMarker.draw_text(ass_center, "id:" + std::to_string(ass_data.objs[i].id).substr(0,3), i, assmarker, white);
...
void v2x_obj_callback(const custom_msgs::ObjectArray &v2x_data)
{
  visualization_msgs::MarkerArray v2x_markers;
  for (int i = 0; i < v2x_data.objs.size(); i++) {
    visualization_msgs::Marker marker;
    marker.header.frame_id ="map";
    marker.header.stamp = ros::Time::now();
    marker.type = visualization_msgs::Marker::MESH_RESOURCE;
    marker.action = visualization_msgs::Marker::ADD;
    marker.mesh_resource = "package://vehicle_description/urdf/CAR_original.dae";
    marker.id = 0;
    marker.pose.position.x = v2x_data.objs[i].x_pos;
    marker.pose.position.y = v2x_data.objs[i].y_pos;
    ...
    v2x_markers.markers.push_back(marker);
  }
  v2x_car_model_pub.publish(v2x_markers);
}
...
int main(int argc, char *argv[])
{
  ...
  ros::Subscriber v2x_obj_sub = nh.subscribe("/other_car_stat", 1, v2x_obj_callback);
  ...
  v2x_car_model_pub = nh.advertise<visualization_msgs::MarkerArray>("/v2x_car_model",1,true);
  ros::spin();
  return 0;
}
