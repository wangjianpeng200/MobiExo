...
void pure_set_ref_path(const std::vector<float> &x,const std::vector<float> &y){
    ...
    for (uint32_t i = 0;i < x.size();i++){
        temp_point << x[i],y[i];
        //std::cout << "[" << temp_point[0] << "," << temp_point[1] << "]" << std::endl;
...
static float set_La_f(const float speed){
   if(speed < 10)                                 return 5;
    else if(10 <= speed && speed < 17)   return 2.0 * speed/3.6;
    else                                            return 16;


   // if(speed < 10)                                 return 4;
    //else if(10 <= speed && speed < 17)   return 5;
    //else   return 6;
...
static float ackermann_steering(const Eigen:: Vector2f &goal_point,const float La){
    ...
    if(fabs(deltai)<=25)                           return deltai*21.2f;
    else if (fabs(deltai)>25 && fabs(deltai)<=30)  return deltai*19.33f;
    else                                           return deltai*15;
}
 ...
 */
float pure_pursuit(){
    //if(ref_path_veh.size() <5) return 0;
